...
void lectura(vector<int> doctors_schedule_ref[5], int &M_maquinas_o, int &N_pacientes_o, int &D_doctores_o, int &P_r_o, int &P_p_o, int &P_u_o, string name_file);
...
void write(int** schedule_pac_doctor, int** schedule_pac_machine, int N_pacientes, string name_file);
...
int main() {
    string name_file = "Caso4";
    int M_maquinas=0; int N_pacientes=0; int D_doctores=0; 
    int P_r=0; int P_p=0; int P_u=0;
    vector<int> doctors_schedule_ref[5];
    lectura(doctors_schedule_ref, M_maquinas, N_pacientes, D_doctores, P_r, P_p, P_u, name_file);
    ...
    cout << "Greedy initializated" << endl;
    ...
    int max_size = 47;
    int stop_criterion = 100;
    cout << "TabuSearch initializated" << endl;
    Tabu(schedule_pac_doctor, schedule_pac_machine, max_size, P_u,P_p,P_r, stop_criterion);
    cout << "TabuSearch finished!" << endl;
    write(schedule_pac_doctor, schedule_pac_machine, N_pacientes, "output");
...
void maquina_queue(queue<int> &Q_Maq, int M_maquinas) {
    clear(Q_Maq);
    for (int Maq = 0; Maq < M_maquinas; Maq++) {
        Q_Maq.push(Maq+1);
    }
}
...
void paciente_queue(queue<int> &Q_paciente, vector<int> &L_pacientes, int day,  int P_r, int P_p, int P_u) {
    clear(Q_paciente);
    int size = L_pacientes.size();
    for (int paciente = 0; paciente < size; paciente++){
...
void write(int **schedule_pac_doctor, int **schedule_pac_machine, int N_pacientes, string name_file) {
    ...
    for (int paciente_i = 0; paciente_i <  N_pacientes; paciente_i++) {
        for (int doctor = 0; doctor < 20*16; doctor++)
        {
...
void Prettywrite(int **schedule_pac_doctor, int **schedule_pac_machine, int N_pacientes, int n_file) {
    ...
    {    
        ...
        for (int paciente_i = 0; paciente_i <  N_pacientes; paciente_i++) {
            ...
            for (int doctor = day*16; doctor < (day+1)*16; doctor++)
            {
                myfile << " " << schedule_pac_doctor[paciente_i][doctor] << "  ||";
            }
        myfile << "\n";
...
int f_ev(int** sol_d, int** sol_m, int N_pac) {
        int same_doctor = 0;
        int same_machine = 0;
        for (int index_pac = 0; index_pac < N_pac; index_pac++) {
            vector<int> doctores_pac;
            vector<int> machines_pac;
            for (int index_doc = 0; index_doc < 20*16; index_doc++) {
...
tuple<int, int> best_N(int **local_best_d, int **local_best_m, int **S_c, int **S_m, queue<tuple<int, int>> &Tabu_list, int U, int P, int R) {
    ...
    int forbidden_day = 0;
    for (int day = 0; day < 20; day++) {
        for (int paciente_i = 0; paciente_i < (U+P+R); paciente_i++){
...
int check_factibility(int **S, int U,int P,int R) {
    ...
    for (int paciente_i = 0; paciente_i < P+U+R; paciente_i++)
    {
        count_sessions = 0;
        fin = 0;
        for (int day = 0; day < 20; day++)
